...
(function() {
  ...
  document.querySelectorAll('#navmenu a').forEach(navmenu => {
    navmenu.addEventListener('click', (e) => {
      if (!navmenu.hash) return; // non-anchor links
      ...
      const closeAndScroll = () => {
        ...
        // Smooth scroll using scroll-margin-top already defined in CSS
      ...
      const isMobileOpen = document.body.classList.contains('mobile-nav-active');
      if (isMobileOpen) {
        mobileNavToggle();
        // wait for drawer transition (~240ms) before scrolling
        setTimeout(closeAndScroll, 260);
      } else {
        closeAndScroll();
      }
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.isotope-layout').forEach(function(isotopeItem) {
    ...
    imagesLoaded(isotopeItem.querySelector('.isotope-container'), function() {
      if (document.body.classList.contains('mobile-nav-active')) return; // avoid heavy relayout during mobile overlay
